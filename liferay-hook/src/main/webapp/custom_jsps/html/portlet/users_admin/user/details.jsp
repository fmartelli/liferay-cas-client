	...
	<aui:fieldset cssClass="span5">
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<aui:input name="birthdayDay" type="hidden" value="30" />
        ...
        <% HashMap<String, String> principal = (HashMap) request.getSession().getAttribute("principal"); %>
        <aui:input name="ID" type="resource" value='<%= principal.get("uid") %>' />
		<aui:input name="Common Name" type="resource" value='<%= principal.get("cn") %>' />
		<aui:input name="Name" type="resource" value='<%= principal.get("givenName") %>' />
		<aui:input name="Surname" type="resource" value='<%= principal.get("sn") %>' />
		<aui:input name="Email" type="resource" value='<%= principal.get("mail") %>' />
